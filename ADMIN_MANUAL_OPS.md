...
|---|---|
| **SLA** | *Service Level Agreement* — Engagement de délai de résolution. Définit le temps maximum acceptable pour traiter un dossier selon son urgence : HIGH < 3h, MEDIUM < 5h, LOW < 10h. Si le dossier est clôturé dans ce délai, le SLA est respecté. |
| **KPI** | *Key Performance Indicator* — Indicateur clé de performance. Chiffre synthétique qui mesure un aspect précis de l'activité (ex: satisfaction, délai, volume). |
| **WAT** | *West Africa Time* — Fuseau horaire UTC+1, utilisé à Douala (Cameroun). Toutes les dates et le calcul "aujourd'hui" s'appuient sur ce fuseau. |
| **Urgence HIGH** | Dossier critique — délai cible de résolution : **< 3 heures**. |
| **Urgence MEDIUM** | Dossier standard — délai cible de résolution : **< 5 heures**. |
| **Urgence LOW** | Dossier faible priorité — délai cible de résolution : **< 10 heures**. |
| **PENDING** | Statut : dossier soumis par le client, pas encore pris en charge par un agent. |
| **IN_PROGRESS** | Statut : dossier pris en charge, traitement en cours. |
| **TREATED / CLOSED** | Statut : dossier clôturé après réponse finale. |
| **Avg resolution** | Temps moyen entre la création du dossier et sa clôture. |
| **1st response** | Temps entre la création du dossier et le premier message envoyé par un agent (hors notes internes). Mesure la réactivité. |
| **Msgs/ticket** | Nombre moyen de messages échangés par dossier (hors notes internes). |
| **Score composite** | Score 0–100 calculé sur 50% satisfaction + 50% vitesse de résolution. Utilisé dans le graphe "Global performance by urgency". |
...
#### Avg resolution (h) (courbe)
Temps moyen de résolution dans le temps — même axe X auto-adaptatif. Complète les 3 graphes précédents pour donner une vue complète de l'évolution de l'équipe.
...
Dernière ligne = **TEAM AVERAGE** (fond bleu foncé) — valeurs de référence pour l'équipe entière. Les chiffres individuels de chaque agent se comparent à cette ligne.

**Règle de couleur sur la colonne Satisfaction :**
- 🟢 Vert = satisfaction de l'agent ≥ moyenne équipe
- 🔴 Rouge = satisfaction de l'agent < moyenne équipe

**Règle de couleur sur SLA % :**
- 🟢 Vert = ≥ 80%
- 🟠 Orange = 60–79%
- 🔴 Rouge = < 60%
