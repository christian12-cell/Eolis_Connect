...
interface Props {
  ...
  ticketId?: string
  onCostUpdate?: (costFcfa: number) => void
  className?: string
  disabled?: boolean
  size?: 'sm' | 'md'
  label?: string
...
export function VoiceRecorder({ onResult, ticketId, onCostUpdate, className = '', disabled, size = 'md', label }: Props) {
      ...
      mr.onstop = async () => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (data.costFcfa && onCostUpdate) onCostUpdate(data.costFcfa)
          }
        } finally {
          setRecState('idle')
        }
  ...
  if (recState === 'transcribing') {
    return (
      <button disabled type="button" className={`flex items-center gap-1.5 ${className}`}>
        <Loader2 size={iconSize} className="animate-spin text-[#4A8FC4]" />
